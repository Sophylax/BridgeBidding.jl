"""
    SupervisedValueEvaluator(trainingset, testingset; onnewbest = nothing)

Evaluator that calculates the distance of predicted values, when given supervised datasets.

Optionally with *onnewbest*, it will track the best performance and call *onnewbest()* when a new best in test set is reached.

    (sme::SupervisedValueEvaluator)(model)
