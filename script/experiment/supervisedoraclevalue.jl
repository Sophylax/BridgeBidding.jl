...
function main(args::Vector{String})
    s = ArgParseSettings()
    s.description = "Supervised Value Experiment for Bridge Bidding"
...
function main(;gamesfile::String, epochs::Int=10, batchsize::Int=64,
    ...
    @info "Bridge Bidding Supervised Value Experiment"
    if train
    ...
    dtrn, dtst = generate_supervised_oraclemultiset(gamesfile, split_ratios=[9,1], batchsize=batchsize)
    @info "Data loaded" length(dtrn) length(dtst)
    
    savefun = () -> Knet.save(modelfile, "model", model)

    evaluator = SupervisedValueEvaluator(take(dtrn,div(length(dtrn),10)), dtst, onnewbest = savefun)
