...
function main(args::Vector{String})
    s = ArgParseSettings()
    s.description = "REINFORCE Experiment for Bridge Bidding"
...
function main(;dummyfile::String, epochs::Int=10, batchsize::Int=64, fixedsides::String="",
    ...
    @info "Bridge Bidding REINFORCE Experiment"
    if train
    ...
    fixedmodel = deepcopy(model);
    for param in Knet.params(model); param.opt = Adam(lr=learningrate); end;
    ...
    if fixedsides != ""
        ...
        for ch in fixedsides
            ...
            actors[idx] = fixedmodel
        end
    end
    
    @info "Warming up the game engine"
    playepisode(ddtst.games[1:batchsize], actormodels=fill(model, 4))

    gse = GameScoreEvaluator(ddtst);
    ae = AdversarialEvaluator(ddtst);
    evalfun = ()->begin gse(model); ae(model, fixedmodel); end

   if train

        trainer = Reinforce(model, ddtrn, actors=actors)
