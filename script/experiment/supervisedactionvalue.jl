...
function main(args::Vector{String})
    s = ArgParseSettings()
    s.description = "Supervised Action-Value Experiment for Bridge Bidding"
    s.exc_handler=ArgParse.debug_handler

    @add_arg_table s begin
        ("--gamesfile"; arg_type=String; default="../../data/enrichedgames.json"; help="location of the supervised games data")
...
function main(;gamesfile::String, epochs::Int=10, batchsize::Int=64,
    ...
    @info "Bridge Bidding Supervised Action-Value Experiment"
    if train
    ...
    dtrn, dtst = generate_supervised_multiset(gamesfile, split_ratios=[9,1], batchsize=batchsize)
    @info "Data loaded" length(dtrn) length(dtst)
    
    savefun = () -> Knet.save(modelfile, "model", model)

    evaluator = SupervisedActionValueEvaluator(take(dtrn,div(length(dtrn),10)), dtst, onnewbest = savefun)
