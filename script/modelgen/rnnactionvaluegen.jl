...
function main(args::Vector{String})
	s = ArgParseSettings()
    s.description = "RNN Multi-Head Model Generator for Bridge Bidding"
...
function main(;cardembed::Int=32, bidembed::Int=64, vulembed=64, lstmhidden::Int=64,
	...
	@info "RNN Multi-Head Model Generator for Bridge Bidding"
    ...
    if lstmdropout + sum(mlpdropout) > 0
        model = DropoutRnnMlpActionValueModel(cardembed=cardembed, bidembed=bidembed, vulembed=vulembed,
                            lstmhidden=lstmhidden, lstmlayers=lstmlayers, mlphidden=mlphidden,
                            lstmdropout=lstmdropout, mlpdropout=mlpdropout)
    else
        model = RnnMlpActionValueModel(cardembed=cardembed, bidembed=bidembed, vulembed=vulembed,
                            lstmhidden=lstmhidden, lstmlayers=lstmlayers, mlphidden=mlphidden)
    end
